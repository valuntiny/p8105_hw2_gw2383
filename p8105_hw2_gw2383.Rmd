...
The number of distinct stations serve the A train is `r sum(distinct(subway_tidy_df, line, station_name, route_name)$route_name == "A")`. Of the stations that serve the A train, `r sum(subway_tidy_ada$ada[which(subway_tidy_ada$route_name == "A")] == TRUE)` stations are ADA compliant.
